...
## More [here](https://r-spatial.github.io/sf/reference/tidyverse.html)
```{r message=FALSE, warning=FALSE, paged.print=FALSE, cache = TRUE}
studysites %>% 
...
```{r message = FALSE, warning = FALSE, paged.print = FALSE, fig.align = 'center',fig.dim = c(15, 7), dpi = 300, eval = FALSE}
library(mapview)
covariates %>% mutate(bearpresence = as_factor(bearpresence)) %>% mapview(alpha = 1, zcol = "bearpresence")

covariates %>% mutate(forestcover = as.numeric(forestcover))%>% mapview(alpha = 1, zcol = "forestcover")


#  mapview(zcol = "geometry")
  mutate(forestcover = as.numeric(forestcover)) %>%
  mapview(zcol = "forestcover")
  scale_fill_viridis_c(
    labels = scales::percent_format(), # format percentage
    name = 'Cover',
    alpha = 0.7) +  # control transparency
